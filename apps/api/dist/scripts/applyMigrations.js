...
async function main() {
    ...
    const existingRows = await prisma.$queryRawUnsafe(`SELECT "migration_name", "finished_at" FROM "_prisma_migrations"`);
    const appliedNames = new Set(existingRows.filter((row) => row.finished_at !== null).map((row) => row.migration_name));
    const incomplete = existingRows.find((row) => row.finished_at === null);
    if (incomplete) {
        throw new Error(`Migration "${incomplete.migration_name}" is recorded as started but never finished. ` +
    ...
    for (const dir of dirs) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            await tx.$executeRawUnsafe(`INSERT INTO "_prisma_migrations"
          ("id", "checksum", "finished_at", "migration_name", "started_at", "applied_steps_count")
         VALUES ($1, $2, now(), $3, now(), $4)`, id, checksum, dir, statements.length);
        });
        appliedCount += 1;
