 ...
 */
async function creditApprovedDeposit(tx, deposit) {
...
export async function createManualDeposit(userId, input) {
...
export async function createPendingGatewayDeposit(params) {
    let methodTitle = params.gatewayProvider;
...
export async function listDepositsForUser(userId, page, pageSize) {
    const where = { userId };
...
export async function listDepositsForAdmin(page, pageSize, status) {
...
export async function reviewDeposit(depositId, reviewerId, action, note) {
    return prisma.$transaction(async (tx) => {
        ...
        if (deposit.status !== "PENDING")
            throw AppError.conflict("Deposit has already been reviewed");
 ...
 */
export async function confirmGatewayDeposit(gatewayRef, result, 
