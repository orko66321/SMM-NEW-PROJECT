 ...
 */
export async function purchasePackageInTransaction(tx, userId, input, idempotencyKey) {
 ...
 */
export async function purchasePackageOrRedirect(userId, input, idempotencyKey) {
 ...
 */
export async function fulfillStorePackageIntent(tx, intent) {
    if (!intent.packageId) {
    ...
    try {
        const result = (await purchasePackageInTransaction(tx, intent.userId, { packageId: intent.packageId, buyerInput: intent.link }, intent.idempotencyKey));
        await tx.orderIntent.update({
            where: { id: intent.id },
