...
function detectPlatform(categoryName) {
...
export async function previewProviderImport(providerId) {
    ...
    const items = remoteServices.map((entry) => {
 ...
 */
export async function bulkImportProviderServices(providerId, input) {
    const provider = await getProviderOrThrow(providerId);
    const remoteServices = await listProviderServices(provider);
    ...
    const parsedByServiceId = new Map();
    ...
    const neededCategoryNames = new Map(); // lowercase -> original casing
    ...
    const rows = Array.from(parsedByServiceId.entries()).map(([id, parsed]) => {
