...
export async function getGatewayAutoVerify(provider) {
...
export async function listEnabledGatewayKeys() {
...
export async function upsertGatewayConfig(provider, input) {
...
export async function getEnabledGatewayCredentials(provider) {
    const config = await prisma.paymentGatewayConfig.findUnique({ where: { provider } });
    if (!config || !config.enabled) {
        throw AppError.badRequest(`${provider} is not enabled`);
    }
    return JSON.parse(decrypt(config.credentialsCiphertext));
}
