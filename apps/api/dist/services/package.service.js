...
export async function listPackagesPublic(productId) {
...
export async function listPackagesForAdmin(page, pageSize, productId) {
...
async function assertPoolIdsExist(poolIds) {
    if (poolIds.length === 0)
        return;
    const count = await prisma.stockPool.count({ where: { id: { in: poolIds } } });
    if (count !== poolIds.length)
        throw AppError.badRequest("One or more stockPoolIds do not exist");
}
export async function createPackage(input) {
    const product = await prisma.product.findUnique({ where: { id: input.productId } });
    if (!product)
        throw AppError.badRequest("Unknown productId");
...
export async function updatePackage(id, input) {
    ...
    const { stockPoolIds, ...data } = input;
    if (stockPoolIds)
        await assertPoolIdsExist(stockPoolIds);
