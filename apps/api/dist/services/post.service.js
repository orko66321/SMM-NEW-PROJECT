...
function resolveYouTubeId(youtubeUrl) {
    const raw = youtubeUrl?.trim();
    if (!raw)
        return null;
    const id = parseYouTubeId(raw);
    if (!id)
        throw AppError.badRequest("Enter a valid YouTube link");
    return id;
}
...
function toRow(input) {
    ...
    if (youtubeUrl !== undefined)
        data.youtubeVideoId = resolveYouTubeId(youtubeUrl);
    return data;
}
export async function listPostsForAdmin(page, pageSize, category, status) {
    const where = {
        ...(category ? { category } : {}),
        ...(status ? { status } : {}),
    };
...
export async function listPublishedPostsPublic(category) {
