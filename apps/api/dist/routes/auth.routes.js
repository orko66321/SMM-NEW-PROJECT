...
import { getPublicUserById, googleAuth, loginUser, logoutSession, refreshSession, registerUser, } from "../services/auth.service.js";
...
authRouter.post("/google", authLimiter, validate(googleAuthSchema), asyncHandler(async (req, res) => {
...
authRouter.post("/forgot-password", authLimiter, validate(forgotPasswordSchema), asyncHandler(async (req, res) => {
    await requestPasswordReset(req.body);
    // Always 204 regardless of whether the account exists — see
    // passwordReset.service.ts's account-enumeration comment.
    res.status(204).end();
}));
authRouter.post("/reset-password", authLimiter, validate(resetPasswordSchema), asyncHandler(async (req, res) => {
