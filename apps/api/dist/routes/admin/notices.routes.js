...
adminNoticesRouter.get("/", asyncHandler(async (_req, res) => {
    res.json({ items: await listNoticesForAdmin() });
}));
adminNoticesRouter.post("/", validate(noticeInputSchema), asyncHandler(async (req, res) => {
...
adminNoticesRouter.put("/:id", validate(noticeObjectSchema.partial()), asyncHandler(async (req, res) => {
...
adminNoticesRouter.delete("/:id", asyncHandler(async (req, res) => {
