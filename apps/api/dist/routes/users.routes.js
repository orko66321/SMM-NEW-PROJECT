...
usersRouter.patch("/me", validate(updateProfileSchema), asyncHandler(async (req, res) => {
    res.json({ profile: await updateProfile(req.user.id, req.body) });
}));
usersRouter.post("/me/password", authLimiter, validate(changePasswordSchema), asyncHandler(async (req, res) => {
    await changePassword(req.user.id, req.body);
    res.status(204).end();
}));
// Returns the plaintext key exactly once — see apiKey.service.ts.
usersRouter.post("/me/api-key", asyncHandler(async (req, res) => {
    const { apiKey, prefix } = await generateApiKey(req.user.id);
    res.status(201).json({ apiKey, prefix });
}));
usersRouter.delete("/me/api-key", asyncHandler(async (req, res) => {
