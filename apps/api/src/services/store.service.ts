...
import type { PurchasePackageInput } from "@smm/shared";
...
function requestFingerprint(input: PurchasePackageInput): string {
 ...
 */
async function claimStockCode(tx: Prisma.TransactionClient, poolIds: string[]): Promise<{ id: string } | null> {
...
export async function getDeliveredCodeForOrder(userId: string, orderId: string): Promise<string> {
  ...
  if (!order.stockCode) throw AppError.notFound("No stock code was delivered for this order");
