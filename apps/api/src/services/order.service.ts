...
export async function fulfillOrderIntent(
  tx: Prisma.TransactionClient,
  intent: { id: string; userId: string; serviceId: string | null; link: string; quantity: number; idempotencyKey: string; expiresAt: Date },
) {
...
export async function recordOrderProviderError(orderId: string, err: unknown) {
...
type ProviderRow = { id: string; apiUrl: string; apiKeyCiphertext: string };
 ...
 */
async function submitToProviders(
  providerServiceId: string,
  candidates: readonly ProviderRow[],
  params: { link: string; quantity: number },
): Promise<string> {
