...
import type { BrandInput } from "@smm/shared";
...
export async function getBrandPublic(id: string) {
  const brand = await prisma.brand.findFirst({ where: { id, isActive: true } });
  if (!brand) throw AppError.notFound("Brand not found");
  return brand;
}

export async function listBrandsForAdmin(page: number, pageSize: number) {
