...
const DEFAULT_LABELS: Record<SupportChannelType, string> = {
  WHATSAPP: "WhatsApp",
  TELEGRAM: "Telegram",
  MESSENGER: "Messenger",
  CUSTOM: "Link",
...
const DEFAULT_SORT: Record<SupportChannelType, number> = {
  WHATSAPP: 0,
  TELEGRAM: 1,
  MESSENGER: 2,
  CUSTOM: 3,
...
async function ensureChannels() {
  await prisma.$transaction(
    SupportChannelTypeValues.map((type) =>
      prisma.supportChannel.upsert({
        where: { type },
        update: {},
 ...
 */
function buildHref(type: SupportChannelType, value: string | null): string | null {
  const v = (value ?? "").trim();
  if (!v) return null;
  ...
  switch (type) {
    ...
    case "TELEGRAM": {
      ...
      if (/^https?:\/\//i.test(v)) return /^https:\/\/t\.me\//i.test(v) ? v : null;
...
export async function listSupportChannelsForAdmin(): Promise<AdminSupportChannel[]> {
...
export async function updateSupportChannel(
  ...
  input: SupportChannelUpdateInput,
): Promise<AdminSupportChannel> {
  await ensureChannels();
 ...
 */
export async function listPublicSupportChannels(): Promise<PublicSupportChannel[]> {
  await ensureChannels();
  ...
  const out: PublicSupportChannel[] = [];
  for (const r of rows) {
