...
import type { StockPoolInput } from "@smm/shared";

export async function listStockPoolsForAdmin(page: number, pageSize: number) {
...
export async function createStockPool(input: StockPoolInput) {
  const existing = await prisma.stockPool.findUnique({ where: { name: input.name } });
  if (existing) throw AppError.conflict("A stock pool with this name already exists");
  return prisma.stockPool.create({ data: input });
}

export async function deleteStockPool(id: string) {
  const pool = await prisma.stockPool.findUnique({ where: { id } });
  if (!pool) throw AppError.notFound("Stock pool not found");
...
export async function bulkAddStockCodes(poolId: string, rawCodes: string) {
  ...
  const lines = rawCodes
    ...
    .map((l) => l.trim())
    .filter((l) => l.length > 0);
  if (lines.length === 0) throw AppError.badRequest("No codes found — enter one code per line");
...
export async function listStockCodesForAdmin(poolId: string, page: number, pageSize: number) {
  const where = { poolId };
...
export async function revokeStockCode(id: string) {
  const code = await prisma.stockCode.findUnique({ where: { id } });
  if (!code) throw AppError.notFound("Stock code not found");
