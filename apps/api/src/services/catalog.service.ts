...
export async function deleteService(id: string) {
  ...
  if (orders > 0 || orderIntents > 0 || dripFeeds > 0 || products > 0) {
    throw AppError.conflict(
      "This service has orders or other records referencing it — disable it instead of deleting",
    );
  }
 ...
 */
export async function recomputeServiceCompletionStats(
 ...
 */
export async function listServiceCompletedOrders(serviceId: string, page: number, pageSize: number) {
  const where = {
    serviceId,
    status: { in: [...COMPLETED_STATUSES] },
    completedAt: { not: null },
