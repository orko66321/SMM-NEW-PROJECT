...
import type { CouponInput } from "@smm/shared";
...
function computeBonus(coupon: { type: string; value: Prisma.Decimal }, amount: number): Prisma.Decimal {
  if (coupon.type === "PERCENT") return new Prisma.Decimal(amount).mul(coupon.value).div(100);
  return coupon.value;
}
 ...
 */
export async function validateCoupon(code: string, userId: string, amount: number) {
  const coupon = await prisma.coupon.findUnique({ where: { code } });
  if (!coupon || !coupon.active) throw AppError.badRequest("Invalid or inactive coupon code");
  ...
  if (alreadyRedeemed) throw AppError.badRequest("You have already used this coupon");
 ...
 */
export async function redeemCouponForDeposit(
  tx: Prisma.TransactionClient,
  params: { couponId: string; userId: string; depositId: string; amount: number },
