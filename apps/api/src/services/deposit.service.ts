    ...
    if (result.status === "PAID") {
      ...
      if (current.orderIntentId) {
        try {
          ...
          logger.error(
            { err, depositId: current.id, orderIntentId: current.orderIntentId, gatewayRef, gatewayProvider: result.gatewayProvider },
            "Order-intent fulfillment threw during a confirmed payment — the deposit is still approved and the wallet credited; the retry cron and/or an admin need to place this order by hand",
          );
        }
      }
