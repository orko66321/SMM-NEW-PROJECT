 ...
 * for an IDOR bug).
 */
async function parseAndAuthorizeOrderIds(userId: string, raw: string | undefined): Promise<string[]> {
  const tokens = (raw ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
function buildAiSubject(subcategoryName: string, orderIds: string[]): string {
