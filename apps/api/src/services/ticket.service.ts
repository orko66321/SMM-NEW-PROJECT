 ...
 */
async function parseAndAuthorizeOrderIds(
  userId: string,
  ...
  if (unique.length === 0) throw AppError.badRequest("Enter at least one Order ID");
  if (unique.length > MAX_ORDER_IDS) {
    throw AppError.badRequest(`At most ${MAX_ORDER_IDS} Order IDs per ticket`);
  }

  const numericTokens = new Map<string, number>(); // original token -> parsed orderNumber
  const cuidTokens: string[] = [];
  ...
  const resolved: { id: string; orderNumber: number }[] = [];
...
async function resolveAutomatedInput(
  userId: string,
  categoryId: string,
  input: { subcategoryId?: string; orderIds?: string },
) {
  if (!input.subcategoryId) throw AppError.badRequest("Please choose a subcategory");
...
function buildAiSubject(subcategoryName: string, orders: { orderNumber: number }[]): string {
  const suffix = orders.length > 1 ? ` (+${orders.length - 1} more)` : "";
  return `${subcategoryName} — Order #${orders[0]!.orderNumber}${suffix}`;
}

function buildHumanSubject(message: string): string {
...
export async function createTicket(userId: string, input: CreateTicketInput) {
  ...
  if (!category) throw AppError.badRequest("Unknown ticket category");
  ...
  if (category.isAutomated) {
    ...
    await runTicketAutomation({
      ticketId: ticket.id,
      userId,
      actionKey: subcategory.actionKey as TicketActionKey,
      orderIds,
    });
  ...
  if (!message) throw AppError.badRequest("A message is required for Human Support");
  const ticket = await prisma.ticket.create({
    data: {
      userId,
      categoryId: category.id,
...
export async function listTicketsForAdmin(
  page: number,
  pageSize: number,
  filters: { status?: string; categoryId?: string; subcategoryId?: string } = {},
) {
  const where = {
 ...
 */
export async function addUserMessage(ticketId: string, userId: string, input: TicketReplyInput) {
  const ticket = await getTicketForUser(ticketId, userId);
  if (ticket.status === "CLOSED") throw AppError.badRequest("This ticket is closed — open a new one");
  ...
  const message = (input.message ?? "").trim();
  if (!message) throw AppError.badRequest("Type a message");
  await prisma.$transaction(async (tx) => {
    await tx.ticketMessage.create({
...
const ACTION_KEY_BY_AGENT_ACTION: Record<string, TicketActionKey> = {
  refill: "REFILL",
  cancel: "CANCEL",
  restart: "RESTART",
};
...
export async function runAgentAction(
  ...
  if (!input.orderId) throw AppError.badRequest("An order must be selected for this action");
