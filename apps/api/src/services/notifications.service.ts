...
async function fireAndLog(kind: string, to: string, message: string): Promise<void> {
  try {
...
export async function sendWelcomeSms(user: { username: string; phone: string | null }): Promise<void> {
...
export async function sendAddFundSms(
  user: { username: string; phone: string | null },
  params: { amount: number; balance: number },
): Promise<void> {
  if (!user.phone) return;
  const config = await getSmsConfig();
  if (!config || !config.addFundEnabled) return;
...
export async function sendOrderConfirmationSms(
  ...
  if (!user.phone) return;
  const config = await getSmsConfig();
  if (!config || !config.orderConfirmationEnabled) return;
