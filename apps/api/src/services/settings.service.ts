...
export async function isEmailBroadcastEnabled(): Promise<boolean> {
  const s = await ensureSettings();
  ...
  await prisma.siteSettings.update({
    ...
    data: {
      ...
      smsOrderConfirmationTemplate:
        input.smsOrderConfirmationTemplate === undefined ? undefined : input.smsOrderConfirmationTemplate || null,
      ...
      emailAddFundSuccessSubject:
        input.emailAddFundSuccessSubject === undefined ? undefined : input.emailAddFundSuccessSubject || null,
      emailAddFundSuccessTemplate:
        input.emailAddFundSuccessTemplate === undefined ? undefined : input.emailAddFundSuccessTemplate || null,
      ...
      emailAddFundFailedSubject:
        input.emailAddFundFailedSubject === undefined ? undefined : input.emailAddFundFailedSubject || null,
      emailAddFundFailedTemplate:
        input.emailAddFundFailedTemplate === undefined ? undefined : input.emailAddFundFailedTemplate || null,
      ...(input.emailOrderSuccessEnabled === undefined
        ? {}
        : { emailOrderSuccessEnabled: input.emailOrderSuccessEnabled }),
      emailOrderSuccessSubject:
        input.emailOrderSuccessSubject === undefined ? undefined : input.emailOrderSuccessSubject || null,
      emailOrderSuccessTemplate:
        input.emailOrderSuccessTemplate === undefined ? undefined : input.emailOrderSuccessTemplate || null,
      ...(input.emailOrderFailedEnabled === undefined
        ? {}
        : { emailOrderFailedEnabled: input.emailOrderFailedEnabled }),
      emailOrderFailedSubject:
        input.emailOrderFailedSubject === undefined ? undefined : input.emailOrderFailedSubject || null,
      emailOrderFailedTemplate:
        input.emailOrderFailedTemplate === undefined ? undefined : input.emailOrderFailedTemplate || null,
...
export async function sendTestEmail(to: string): Promise<void> {
  if (!(await isMailConfigured())) {
    throw AppError.badRequest(
      "Email isn't configured — either set BREVO_API_KEY + MAIL_FROM on the API server (recommended on Railway), or fill in and save the Mailjet settings below (API key, secret key, from-address)",
    );
  }
  ...
  try {
    await sendMail(
      to,
      `Mailjet test — ${s.siteName}`,
      "This is a test email from your admin panel. If you received it, email sending is working.",
    );
  } catch (err) {
...
export async function getSmsConfig() {
  ...
  if (s.smsProvider === "MILEJET") {
    if (!s.smsMilejetApiKeyCiphertext || !s.smsMilejetSecretKeyCiphertext || !s.smsMilejetSenderId) return null;
    return {
      ...base,
      provider: "MILEJET" as const,
      milejet: {
  ...
  if (!s.smsApiKeyCiphertext) return null;
  return { ...base, provider: "URONTO" as const, uronto: { apiKey: decrypt(s.smsApiKeyCiphertext) } };
}
