 ...
 */
async function resolveCallbackGatewayRef(key: PaymentGatewayKey, req: Request): Promise<string | undefined> {
  if (key === "BKASH") {
...
function resolveWebhookGatewayRef(key: PaymentGatewayKey, req: Request): string | undefined {
  ...
  if (key === "ZINIPAY") {
    ...
    const ref =
      typeof raw === "string" || typeof raw === "number" ? String(raw).trim() || undefined : undefined;
    if (!ref) {
      logger.warn(
        { gateway: key, bodyKeys: body ? Object.keys(body) : [], queryKeys: Object.keys(req.query) },
        "ZiniPay webhook: could not resolve invoice_id from the payload",
      );
    }
    return ref;
  }
...
paymentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { amount, paymentMethodId, couponCode, orderIntentId } = req.body as {
      amount: number;
