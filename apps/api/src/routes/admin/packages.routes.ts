...
adminPackagesRouter.get(
  "/",
  validate(packageListQuerySchema, "query"),
...
adminPackagesRouter.get(
  "/:id",
  asyncHandler(async (req, res) => res.json({ package: await getPackageForAdmin(req.params.id!) })),
);

adminPackagesRouter.post(
  "/",
  validate(packageObjectSchema),
  asyncHandler(async (req, res) => {
...
adminPackagesRouter.put(
  "/:id",
  validate(packageObjectSchema.partial()),
  asyncHandler(async (req, res) => {
